...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if(func_name!="upper"&&func_name!="lower")throw Exception(fmt::format("func call {} not supported in planner yet", func_name));



  if(args.size()!=1)throw Exception(fmt::format("need one args", func_name));

  auto arg=args.front();
  if(arg->GetReturnType()!= TypeId::VARCHAR)throw Exception("expect the first arg to be varchar");
  AbstractExpressionRef ref= nullptr;
  if(func_name=="upper")ref= std::make_shared<StringExpression>(args.front(),StringExpressionType::Upper);
  else ref= std::make_shared<StringExpression>(args.front(),StringExpressionType::Lower);
